        ...
        <table className="w-full min-w-[1000px] text-left border-collapse">
          <thead>
            <tr className="bg-[#F2F2F2] border-b border-gray-200">
              ...
              <th className="p-6 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">PERIODE</th>
              <th className="p-6 w-40 text-[10px] font-black text-gray-400 uppercase tracking-widest">ROOM TYPE</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {data.map((item) => (
              <tr key={item.id} className="bg-white hover:bg-[#FDFDFD] transition-all group cursor-pointer" onClick={() => onView?.(item)}>
                ...
                <td className="p-6 text-center text-[11px] font-medium text-black">
                    {item.checkInDate} <span className="text-gray-300 mx-1">➜</span> {item.checkOutDate}
                </td>
                <td className="p-6 text-[11px] font-black text-black uppercase">{item.roomType}</td>
